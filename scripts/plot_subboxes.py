#!/usr/bin/env python3
"""
Plot 2PCF results from pipeline_subboxes.py.

Lines show the mean over 64 subboxes; shaded bands show ±1σ (cosmic variance).
Covariance figures show the normalised correlation matrix derived from the
per-subbox multipole arrays.

Run from any node (no srun needed):
  python scripts/plot_subboxes.py

Output: plots/subbox_*.png
"""
...
# ── load multipoles ────────────────────────────────────────────────────────────
def load(stem):
    d = np.load(DATA_DIR / f'{PREFIX}_multipoles_{stem}.npz')
    return {
        's':       d['s'],
...
SUBTITLE = '64 subboxes of 500 Mpc/h  —  cosmic variance'
...
ax.set_title(f'Data monopole per ASTRA quantile\n{SUBTITLE}')
...
ax.set_title(f'Data quadrupole per ASTRA quantile\n{SUBTITLE}')
ax.legend(title='Q1=underdense → Q4=overdense')
ax.set_xlim(0, 150)
fig.tight_layout()
...
fig.suptitle(f'ASTRA data quantile 2PCF  —  {SUBTITLE}', y=1.01)
fig.tight_layout()
savefig(fig, 'data_multipoles_all_quantiles.png')

# ── Figure 4: random monopole ─────────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(7, 5))
...
ax.set_xlabel(r'$s\ [h^{-1}\,\mathrm{Mpc}]$')
ax.set_ylabel(r'$s^2\,\xi_0(s)\ [h^{-2}\,\mathrm{Mpc}^2]$')
ax.set_title(f'Random monopole per ASTRA quantile\n{SUBTITLE}')
ax.legend(title='Q1=underdense → Q4=overdense')
ax.set_xlim(0, 150)
fig.tight_layout()
savefig(fig, 'rand_monopole_per_quantile.png')

# ── Figure 5: random quadrupole ───────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(7, 5))
for r, label, color in zip(rand_q, LABELS, COLORS):
...
ax.set_xlabel(r'$s\ [h^{-1}\,\mathrm{Mpc}]$')
ax.set_ylabel(r'$s^2\,\xi_2(s)\ [h^{-2}\,\mathrm{Mpc}^2]$')
ax.set_title(f'Random quadrupole per ASTRA quantile\n{SUBTITLE}')
ax.legend(title='Q1=underdense → Q4=overdense')
ax.set_xlim(0, 150)
fig.tight_layout()
savefig(fig, 'rand_quadrupole_per_quantile.png')

# ── Figure 6: data vs randoms monopole ────────────────────────────────────────
...
for idx, ax in enumerate(axes.flat):
    ...
    ax.legend(fontsize=8)
    ax.set_xlim(0, 150)
fig.suptitle(f'Data vs ASTRA-random monopole per quantile  —  {SUBTITLE}', y=1.01)
fig.tight_layout()
...
# ── Figure 8: cross full data × data quantiles ────────────────────────────────
fig, axes = plt.subplots(1, 2, figsize=(12, 5))
...
fig.suptitle(f'Cross-correlation: full data × ASTRA data quantiles  —  {SUBTITLE}', y=1.01)
fig.tight_layout()
savefig(fig, 'cross_full_data_quantiles.png')

# ── Figure 9: cross full data × random quantiles ──────────────────────────────
fig, axes = plt.subplots(1, 2, figsize=(12, 5))
...
for ax, title, ell in zip(axes, ['Monopole', 'Quadrupole'], [0, 2]):
    ...
    ax.set_xlim(0, 150)
    ax.legend(title='Q1=underdense → Q4=overdense', fontsize=8)
fig.suptitle(f'Cross-correlation: full data × ASTRA random quantiles  —  {SUBTITLE}', y=1.01)
fig.tight_layout()
