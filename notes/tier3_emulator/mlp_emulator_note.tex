...
inference-grade near LCDM ($\sim$1$\times$CV), the only true limiters are
\emph{cosmology coverage} (extrapolation, $\to$ the campaign) and \emph{quadrupole
iteration noise} ($\to$ a $\geq$10-iteration run). The SUNBIRD-style inference
pipeline is built and its machinery validated on synthetic mocks; a clean real
recovery is gated by emulator realism and the absence of independent-phase mocks.

\textbf{Minimal coverage run (in progress).} Because coverage is the gate and HODs
plateau by $\sim$30, the minimal step that fits the available allocation is
\textbf{all 52 cosmologies $\times$ 20 maximin HODs $\times$ 3 iterations} (the 10
pilot cosmologies reused), $\sim$840 new runs, $\sim$490 node-hours on the regular
queue (\code{select\_tier3\_min.py}, \code{queue/launch\_tier3\_minimal.sh}). The
payoff test: does the broad-prior monopole LOCO drop from $\sim$18$\times$ toward
$\sim$1$\times$CV as the cosmology axis fills from 10 to 52?

\textbf{Remaining levers.} (1) the full \code{c130}--\code{c181}$\times$100$\times$10
campaign (free on the overrun queue) for coverage \emph{and} quadrupoles; (2) HOD
marginalisation (currently fixed); (3) an external pre-populated mock suite for an
honest covariance and genuinely independent recovery mocks.
