...
\section{Results}
\emph{(To be populated when the first run \code{55159247} completes.)}  The planned
figures are
\begin{itemize}
  ...
  \item \code{marked\_correlation\_signed.png}: $M_0(s)$ for the signed mark, per
        statistic.
\end{itemize}
Key checks to record: (i)~the uniform curve coincides with the standard full-data
$\xifull$; (ii)~the void and knot schemes bracket the uniform curve as the
underdense/overdense-emphasised correlations; (iii)~the cross quadrupole carries
RSD signal while the ASTRA-random auto quadrupole is weak; (iv)~$M_0(s)$ departs
from unity where the environment correlates with clustering.
